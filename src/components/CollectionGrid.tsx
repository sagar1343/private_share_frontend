...
export default function CollectionGrid() {
  ...
  return (
    <div>
      <ul ref={containerRef} className="mt-12 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-4 lg:grid-cols-6 items-center justify-items-center gap-6">
        <li>{actionStatus === CollectionActionStatus.CREATING ? <CollectionCreatingItem /> : <CreateCollectionButton />}</li>
        {collections && collections.map((collection) => <CollectionItem key={collection.id} collection={collection} isActive={active === collection.id && actionStatus !== CollectionActionStatus.RENAMING} onClick={() => setActive(active === collection.id ? null : collection.id)} />)}
      </ul>
      {paginatedCollections && (
        <div className="flex justify-center my-12">
          <Pagination count={paginatedCollections.count} currentPage={page} handleNext={handleNext} handlePrevious={handlePrevious} pageSize={17} />
        </div>
      )}
    </div>
  );
