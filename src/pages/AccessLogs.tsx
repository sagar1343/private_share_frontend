...
export default function AccessLogs() {
  ...
  const { data, isLoading } = useFetch<PaginatedResponse<IAccessLogs>>(["logs", { fileId: id }], `api/files/${id}/logs/?page=${page}`);
  ...
  const filteredLogs = logs.filter((log) => log.access_by.toLowerCase().includes(filter.toLowerCase()));
  ...
  const logsToShow = activeTab === "all" ? [...groupedLogs.today, ...groupedLogs.yesterday, ...groupedLogs.earlier] : activeGroupedLogs[activeTab];

  if (isLoading) return <Loader />;

  if (logs.length === 0) {
    return <p className="mt-12 text-center text-gray-500">No access logs available.</p>;
  }

  return (
    <>
      <AccessLogsTable groupedLogs={activeGroupedLogs} filter={filter} setFilter={setFilter} activeTab={activeTab} setActiveTab={setActiveTab} />

      {logsToShow.length > 0 && (
        <div className="flex justify-center my-12">
          <Pagination count={data?.count!} currentPage={page} handleNext={onNext} handlePrevious={onPrevious} pageSize={12} />
        </div>
      )}
    </>
  );
